...
def save_content_to_file(content: bytes, folder_name: str, file_name: str) -> str:
    ...
    try:
        ...
        for row in csv_reader:
            quoted_row = [f'"{field}"' if not (field.startswith('"') and field.endswith('"')) else field for field in row]
            quoted_lines.append(",".join(quoted_row))
...
def parse_args(
    ...
    if amazon_ads or amazon_fulfillment  or amazon_retail:
        parser.add_argument(
            "--report_list",
            type=str,
            required=True,
